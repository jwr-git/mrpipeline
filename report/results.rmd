...
Results for the main MR analyses ([Wald ratio]((https://mr-dictionary.mrcieu.ac.uk/term/wald-ratio/)) (WR) and [inverse variance weighted]((https://mr-dictionary.mrcieu.ac.uk/term/ivw/)) (IVW)) are given here. The WR method is performed on single-SNP instruments, whilst the IVW method is performed on everything else.
...
```{r forest-plot, echo = F, message = F, warning = F, dpi=300, fig.width=4, fig.height=4}
if (exists("raw.plots")) {
  grid.newpage()
  grid.draw(raw.plots[[as.integer(rownames(plots[plots$type == "forest", ]))]])
}
```
